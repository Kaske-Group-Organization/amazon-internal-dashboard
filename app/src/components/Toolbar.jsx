...
// Dropdown mit position:fixed um overflow:hidden zu umgehen
function FixedDropdown({ anchorRef, open, children, width = 300 }) {
...
export default function Toolbar({ pageTitle = 'Übersicht', onExport }) {
  ...
  const dateLabel  = (dateFrom || dateTo) ? `${fmtDate(dateFrom) ?? '...'} – ${fmtDate(dateTo) ?? '...'}` : null
  ...
  const clearFilter = () => {
    setLocalAsin(''); setLocalName(''); setAsinFilter(''); setShowFilter(false)
  }
  const openFilter = () => {
    const q = asinFilter.trim()
    setLocalAsin(/^B[0-9A-Z]/i.test(q) ? q : '')
    setLocalName(/^B[0-9A-Z]/i.test(q) ? '' : q)
    setShowFilter(true)
    setShowDate(false)
    setShowExport(false)
  }
    ...
    <div className="toolbar">
      ...
      {showSearch ? (
        <div className="toolbar-search-wrap">
          <IconSearch/>
          <input autoFocus type="text" placeholder="ASIN oder Produkt…"
            value={asinFilter} onChange={e=>setAsinFilter(e.target.value)}
      ...
      <div ref={filterRef}>
        <button
          ...
          title="Filter"
          onClick={openFilter}
          style={hasFilter ? {borderColor:'var(--ac)',color:'var(--ac)',background:'var(--blue-bg)',position:'relative'} : {position:'relative'}}
        >
          <IconFilter/>
        ...
        <FixedDropdown anchorRef={filterRef} open={showFilter} width={320}>
          <div style={{...dropdownBase,fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:13,marginBottom:14}}>
            Tabellenfilter
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            <div>
              <label style={{fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.5px',color:'var(--tx3)',display:'block',marginBottom:4}}>
                ASIN(s) – komma- oder zeilengetrennt
              </label>
              <textarea
                placeholder={'z.B. B0010J8ZB6\nB000WQQEX4, B0D8BKCLG3'}
                value={localAsin}
                onChange={e=>{ setLocalAsin(e.target.value); setLocalName('') }}
                rows={3}
                style={{width:'100%',padding:'7px 10px',borderRadius:'var(--r)',border:'1px solid var(--bdr2)',fontSize:12,fontFamily:"'JetBrains Mono',monospace",outline:'none',color:'var(--tx)',resize:'vertical',lineHeight:1.6}}
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text" placeholder="z.B. Pflasterlöser"
                value={localName}
          ...
          <div style={{display:'flex',gap:8,marginTop:14}}>
            <button onClick={applyFilter} style={{flex:1,padding:'8px',borderRadius:'var(--r)',border:'none',background:'var(--ac)',color:'#fff',fontFamily:"'Source Sans 3',sans-serif",fontSize:13,fontWeight:600,cursor:'pointer'}}>
              Anwenden
            </button>
            {(localAsin||localName||asinFilter) && (
              <button onClick={clearFilter} style={{padding:'8px 12px',borderRadius:'var(--r)',border:'1px solid var(--bdr2)',background:'var(--sur2)',color:'var(--tx2)',fontFamily:"'Source Sans 3',sans-serif",fontSize:13,cursor:'pointer'}}>
                Löschen
              </button>
            )}
          </div>
      ...
      {/* Export */}
      <div ref={exportRef}>
        <button className="toolbar-icon-btn" title="Exportieren"
          onClick={()=>{ setShowExport(!showExport); setShowDate(false); setShowFilter(false) }}>
          <IconDownload/>
        </button>
        <FixedDropdown anchorRef={exportRef} open={showExport} width={220}>
          <div style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.5px',color:'var(--tx3)',marginBottom:8}}>
            Export Format
          </div>
          {[
            ...
            <button key={label} onClick={()=>{ fn(); setShowExport(false) }}
              style={{display:'flex',alignItems:'center',gap:10,width:'100%',padding:'9px 8px',border:'none',background:'transparent',cursor:'pointer',borderRadius:6,fontSize:13,color:'var(--tx)',fontFamily:"'Source Sans 3',sans-serif",textAlign:'left'}}
