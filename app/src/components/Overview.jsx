...
const KPI_INFO = {
  ...
  sessions:   'Quelle: Brand Analytics (SP-API)\nEinzigartige Besucher-Sessions auf deinen Amazon-Produktseiten.',
...
export default function Overview({ data, onExport }) {
  ...
  // Nutze Nach Datum+ASIN für tagesaktuelle ASIN-Daten
  const rawTraffic = data.traffic?.['Nach Datum+ASIN'] ?? data.traffic?.['Nach ASIN'] ?? []

  const brand   = useMemo(() => filterByDate(rawBrand, 'startDate'), [rawBrand, filterByDate])
  // Datum-Spalte heißt jetzt 'Datum' statt 'Von'
  const traffic = useMemo(() =>
    filterByDate(filterByAsin(rawTraffic, ['Child ASIN','Parent ASIN']), 'Datum')
  , [rawTraffic, filterByDate, filterByAsin])
  ...
  const asinRaw = useMemo(() => {
    ...
    traffic.forEach(r => {
      ...
      if (!map[asin]) map[asin] = { asin, revenue:0, orders:0, sessions:0, buybox:[], rate:[], dates:[] }
      ...
      if (r['Datum']) map[asin].dates.push(r['Datum'])
    })
    return Object.values(map).filter(a=>a.asin!=='–').map(a=>({
      ...
      letztesMonat: a.dates.length  ? fmtMonth(a.dates[a.dates.length-1]) : '–',
    }))
  }, [traffic, getTitle])
  ...
  const exportData = asinTotals.map(a=>({'ASIN':a.asin,'Produktname':a.titel,'Datum':a.letztesMonat,'Umsatz (€)':a.revenue,'Bestellungen':a.orders,'Sessions':a.sessions,'Bestellrate %':a.rate??'','Buy Box %':a.buybox??''}))

  useMemo(() => {
    if (onExport) onExport.current = (format) => {
        ...
        <div className="card">
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:4}}>
              ...
              <InfoTooltip text={'Quelle: Brand Analytics (SP-API)\nTäglicher Produktumsatz im gewählten Zeitraum.'} position="right"/>
            </div>
      ...
      <div className="card">
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem'}}>
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            ...
            <InfoTooltip text={'Quelle: Verkäufe & Traffic / Nach Datum+ASIN (SP-API)\nTagesaktuelle ASIN-Daten. Buy Box % und Bestellrate sind Durchschnittswerte.'} position="right"/>
          </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <Th col="letztesMonat" label="Datum"/>
