...
export default function Advertising({ data }) {
  ...
  const chartData = useMemo(() => {
    const top8 = topTerms.slice(0,8)
    return {
      labels: top8.map(t=>t.query?.slice(0,20)??'–'),
      datasets: [
        { label:'Spend (€)', data:top8.map(t=>+t.spend.toFixed(2)), backgroundColor:'rgba(37,99,235,0.5)' },
        { label:'Sales (€)', data:top8.map(t=>+t.sales.toFixed(2)), backgroundColor:'rgba(22,163,74,0.5)' },
      ]
    }
  },[topTerms])
  ...
  return (
    <div>
      ...
      <div className="card-grid">
        <div className="card card-full"><div className="card-title">Spend vs. Sales – Top Search Terms</div><div className="chart-box"><Bar data={chartData} options={chartOpts} /></div></div>
      </div>
      <div className="card" style={{marginBottom:'1rem'}}>
        <div className="card-title">Search Terms Performance</div>
      ...
      <div className="card-grid">
        <div className="card">
          <div className="card-title">Kampagnen</div>
          <div className="tbl-wrap">
            <table>
